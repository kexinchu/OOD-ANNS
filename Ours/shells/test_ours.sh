...
# Test script for Ours implementation
# Usage: ./test_ours.sh
...
if [ -f "$ROARGRAPH_DATA_DIR/query.train.10M.fbin" ]; then
    ...
    echo "Warning: Train query file not found"
fi
...
if [ -f "$ROARGRAPH_DATA_DIR/train.gt.bin" ]; then
    ...
    echo "Warning: Train GT file not found"
fi

if [ -f "$ROARGRAPH_DATA_DIR/query.10k.fbin" ]; then
    TEST_QUERY_FILE="$ROARGRAPH_DATA_DIR/query.10k.fbin"
elif [ -f "$DATA_DIR/test_query.fbin" ]; then
    TEST_QUERY_FILE="$DATA_DIR/test_query.fbin"
else
    echo "Warning: Test query file not found"
fi

if [ -f "$ROARGRAPH_DATA_DIR/groundtruth-computed.10k.ibin" ]; then
    TEST_GT_FILE="$ROARGRAPH_DATA_DIR/groundtruth-computed.10k.ibin"
elif [ -f "$DATA_DIR/test_gt.ibin" ]; then
    TEST_GT_FILE="$DATA_DIR/test_gt.ibin"
else
    echo "Warning: Test GT file not found"
fi
...
echo "  Test Query: $TEST_QUERY_FILE"
echo "  Test GT: $TEST_GT_FILE"
...
./test/test_ours \
    ...
    --test_query_path "$TEST_QUERY_FILE" \
    --test_gt_path "$TEST_GT_FILE" \
    --metric "$METRIC" \
...
echo "Check $RESULT_DIR/ours_results.csv for results"
