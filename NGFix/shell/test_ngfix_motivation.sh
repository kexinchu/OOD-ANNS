...
# Usage: 
#   ./test_ngfix_motivation.sh [--nohup] [--efs EFS_VALUE] [--test-size small]
#   --nohup: Run in background with nohup
#   --efs: efSearch value (default: 100)
#   --test-size small: Use small test dataset for quick testing
...
efC=500
efC_AKNN=1500
efs=100  # efsearch value for testing
K=100
USE_NOHUP=false
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        *)
            ...
            echo "Usage: $0 [--nohup] [--efs EFS_VALUE] [--test-size small]"
            exit 1
            ;;
    esac
done
...
# Test data - use small dataset if requested
if [ "$USE_SMALL_TEST" = true ]; then
    # Use 1K test queries and limit train queries to 100K for quick testing
    ...
    echo "⚠ Using SMALL test dataset (will limit to 100K train queries in code)"
    echo "⚠ Note: You need to modify test_ngfix_motivation.cc to limit train_number for small test"
else
    # Full dataset
    TEST_QUERY_PATH="${BASE_DATA_DIR}/query.10k.fbin"
    TEST_GT_PATH="${BASE_DATA_DIR}/groundtruth-computed.10k.ibin"
...
if [ "$USE_NOHUP" = true ]; then
    ...
    nohup bash -c "$(declare -f run_test generate_report); run_test && generate_report" > ${LOG_FILE} 2>&1 &
    PID=$!
    echo "Test started in background with PID: $PID"
    echo "Log file: ${LOG_FILE}"
    echo "Results will be saved to: ${RESULT_CSV}"
    echo ""
    echo "To monitor progress: ./shell/monitor.sh motivation --efs ${efs}"
    echo "To check status: ./shell/monitor.sh motivation --efs ${efs} --status"
else
