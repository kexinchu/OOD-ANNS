...
public:
    ...
    HNSW_NGFix(Metric metric, std::string path) {
        ...
        // Resize node_locks - create new vector and assign (shared_mutex is default constructible)
        node_locks = std::vector<std::shared_mutex>(this->max_elements);
        // Reserve space first to avoid reallocation during resize
    ...
    void resize(size_t new_max_elements) {
        ...
        // Ensure entry_point is still valid after resize
        if(entry_point >= max_elements) {
            entry_point = 0; // Reset to first element if invalid
        }
    }
    ...
    void HNSWBottomLayerInsertion(T* data, id_t cur_id, size_t efC) {
        ...
        // Ensure cur_id is within bounds
        ...
        for(auto [_, neighbor_id] : neighbors) {
            // Ensure neighbor_id is within bounds
            ...
            if(sz < M0) {
                ...
                for (int j = st; j < st + sz; j++) {
                    ...
                    // Ensure candidate_id is within bounds
    ...
    */
    ...
    // Search with frontier churn tracking
    // Returns: (results, ndc, frontier_churn)
    // Frontier churn: average replacement rate of frontier sets in last two rounds
    std::tuple<std::vector<std::pair<float, id_t> >, size_t, float> 
    searchKnnWithMetrics(T* query_data, size_t k, size_t ef, size_t& ndc) {
        size_t hop_limit = std::numeric_limits<int>::max();
        size_t hop = 0;
        if(ef < k) {
            hop_limit = ef;
        ...
        // Ensure entry_point is within bounds
        if(entry_point >= max_elements || entry_point >= node_locks.size()) {
            return std::make_tuple(std::vector<std::pair<float, id_t>>(), 0, 0.0f);
        }
        ...
        // Track frontier sets for calculating churn in last two rounds
        std::vector<std::vector<id_t> > frontier_history;
        float frontier_churn = 0.0f;
        ...
        while (!q->is_empty()) {
            ...
            // Capture frontier snapshot at the END of each round (after processing neighbors)
            // Capture every few hops to track changes (every 5 hops to avoid too much overhead)
            if(hop % 5 == 0 || hop <= 10) {
                auto current_frontier = q->get_frontier_snapshot();
                frontier_history.push_back(current_frontier);
                
                // Keep only last 5 frontier sets (for calculating churn of last rounds)
                if(frontier_history.size() > 5) {
                    frontier_history.erase(frontier_history.begin());
                }
            }
        }
        
        // Capture final frontier
        if(frontier_history.empty() || frontier_history.back() != q->get_frontier_snapshot()) {
            auto final_frontier = q->get_frontier_snapshot();
            frontier_history.push_back(final_frontier);
            if(frontier_history.size() > 5) {
                frontier_history.erase(frontier_history.begin());
            }
        }
        
        // Calculate frontier churn: average replacement rate in last two rounds
        if(frontier_history.size() >= 2) {
            float total_churn = 0.0f;
            int churn_count = 0;
            
            // Calculate replacement rate between consecutive frontier sets in last rounds
            // Use last 2-3 frontier sets to calculate average churn
            size_t start_idx = frontier_history.size() >= 3 ? frontier_history.size() - 3 : 0;
            
            for(size_t i = start_idx; i < frontier_history.size() - 1; ++i) {
                auto& frontier_prev = frontier_history[i];
                auto& frontier_curr = frontier_history[i + 1];
                
                // Convert to sets for easier comparison
                std::unordered_set<id_t> prev_set(frontier_prev.begin(), frontier_prev.end());
                std::unordered_set<id_t> curr_set(frontier_curr.begin(), frontier_curr.end());
                
                // Calculate replacement rate: (elements in curr but not in prev) / max(size)
                size_t replaced = 0;
                for(id_t id : curr_set) {
                    if(prev_set.find(id) == prev_set.end()) {
                        replaced++;
                    }
                }
                size_t max_size = std::max(prev_set.size(), curr_set.size());
                if(max_size > 0) {
                    total_churn += (float)replaced / max_size;
                    churn_count++;
                }
            }
            
            if(churn_count > 0) {
                frontier_churn = total_churn / churn_count;
            }
        }
        
        q->releaseVisitedList();
        auto res = q->get_result(k);
        return std::make_tuple(res, ndc, frontier_churn);
    }
